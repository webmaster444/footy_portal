...
function parsePositiveText(t) {
    tags = {};
    var e = {},
        n = d3version3.select("#per-line").property("checked");
    ...
    }), tags = d3version3.entries(tags).sort(function(t, e) {
...
function generatePositive() {
    pLayout.font(d3version3.select("#font").property("value")).spiral(d3version3.select("input[name=spiral]:checked").property("value")), fontSize = d3version3.scale['log']().range([10, 100]), tags.length && fontSize.domain([+tags[tags.length - 1].value || 1, +tags[0].value]), complete = 0, statusText.style("display", null), words = [], pLayout.stop().words(tags.slice(0, max = Math.min(tags.length, 250))).start()
}
...
function draw(t, e) {
    ...
    var n = pVis.selectAll("text").data(words, function(t) {
    ...
    var a = background.append("g").attr("transform", pVis.attr("transform")),
        r = a.node();
    n.exit().each(function() {
        r.appendChild(this)
    }), a.transition().duration(1e3).style("opacity", 1e-6).remove(), pVis.transition().delay(1e3).duration(750).attr("transform", "translate(" + [w >> 1, h >> 1] + ")scale(" + scale + ")")
}
...
! function(t) {
    function e() {
        ...
        var e = [256, 256],
            ...
            x = [],
            w = 1 / 0,
            b = d3version3.dispatch("word", "end"),
            z = null,
            k = {};
        return k.start = function() {
            ...
            return arguments.length ? (p = d3version3.functor(t), k) : p
        }, k.rotate = function(t) {
            return arguments.length ? (g = d3version3.functor(t), k) : g
        }, k.text = function(t) {
            return arguments.length ? (h = d3version3.functor(t), k) : h
        }, k.spiral = function(t) {
            return arguments.length ? (m = M[t + ""] || t, k) : m
        }, k.fontSize = function(t) {
            return arguments.length ? (y = d3version3.functor(t), k) : y
        }, k.padding = function(t) {
            return arguments.length ? (v = d3version3.functor(t), k) : v
        }, d3version3.rebind(k, b, "on")
    }
...
}("undefined" == typeof exports ? d3version3.layout || (d3version3.layout = {}) : exports);
var unicodePunctuationRe = "!-#%-*,-/:;?@\\[-\\]_{}\xa1\xa7\xab\xb6\xb7\xbb\xbf\u037e\u0387\u055a-\u055f\u0589\u058a\u05be\u05c0\u05c3\u05c6\u05f3\u05f4\u0609\u060a\u060c\u060d\u061b\u061e\u061f\u066a-\u066d\u06d4\u0700-\u070d\u07f7-\u07f9\u0830-\u083e\u085e\u0964\u0965\u0970\u0af0\u0df4\u0e4f\u0e5a\u0e5b\u0f04-\u0f12\u0f14\u0f3a-\u0f3d\u0f85\u0fd0-\u0fd4\u0fd9\u0fda\u104a-\u104f\u10fb\u1360-\u1368\u1400\u166d\u166e\u169b\u169c\u16eb-\u16ed\u1735\u1736\u17d4-\u17d6\u17d8-\u17da\u1800-\u180a\u1944\u1945\u1a1e\u1a1f\u1aa0-\u1aa6\u1aa8-\u1aad\u1b5a-\u1b60\u1bfc-\u1bff\u1c3b-\u1c3f\u1c7e\u1c7f\u1cc0-\u1cc7\u1cd3\u2010-\u2027\u2030-\u2043\u2045-\u2051\u2053-\u205e\u207d\u207e\u208d\u208e\u2329\u232a\u2768-\u2775\u27c5\u27c6\u27e6-\u27ef\u2983-\u2998\u29d8-\u29db\u29fc\u29fd\u2cf9-\u2cfc\u2cfe\u2cff\u2d70\u2e00-\u2e2e\u2e30-\u2e3b\u3001-\u3003\u3008-\u3011\u3014-\u301f\u3030\u303d\u30a0\u30fb\ua4fe\ua4ff\ua60d-\ua60f\ua673\ua67e\ua6f2-\ua6f7\ua874-\ua877\ua8ce\ua8cf\ua8f8-\ua8fa\ua92e\ua92f\ua95f\ua9c1-\ua9cd\ua9de\ua9df\uaa5c-\uaa5f\uaade\uaadf\uaaf0\uaaf1\uabeb\ufd3e\ufd3f\ufe10-\ufe19\ufe30-\ufe52\ufe54-\ufe61\ufe63\ufe68\ufe6a\ufe6b\uff01-\uff03\uff05-\uff0a\uff0c-\uff0f\uff1a\uff1b\uff1f\uff20\uff3b-\uff3d\uff3f\uff5b\uff5d\uff5f-\uff65",
    fill = d3version3.scale.category20b(),
    w = 600,
    h = 600,
    words = [],
    max, scale = 1,
    ...
    fetcher, statusText = d3version3.select("#status"),
    pLayout = d3version3.layout.cloud().timeInterval(10).size([w, h]).fontSize(function(t) {
        return fontSize(+t.value)
    }).text(function(t) {
        return t.key
    }).on("word", progress).on("end", draw),
    pSvg = d3version3.select("#best-container").append("svg").attr('viewBox', '0 0 ' + w + ' ' + h).attr('width', '100%').attr("preserveAspectRatio", "xMinYMin meet").attr('height', '100%'),
    background = pSvg.append("g"),
    pVis = pSvg.append("g").attr("transform", "translate(" + [w >> 1, h >> 1] + ")");

var form = d3version3.select("#form").on("submit", function() {
    parsePositiveText(d3version3.select("#positive_text").property("value")), d3version3.event.preventDefault()
});
form.selectAll("input[type=number]").on("click.refresh", function() {
    this.value !== this.defaultValue && (generatePositive(), this.defaultValue = this.value)
}), form.selectAll("input[type=radio], #font").on("change", generatePositive);
...
d3version3.select("#random-palette").on("click", function() {
        paletteJSON("http://www.colourlovers.com/api/palettes/random", {}, function(t) {
            fill.range(t[0].colors), pVis.selectAll("text").style("fill", function(t) {
                return fill(t.text.toLowerCase())
            })
    ...
    function() {
        ...
        function e() {
            ...
            var o = l.selectAll("line.angle").data(d3version3.range(c).map(h));
            ...
            s.enter().append("path").attr("class", "drag").attr("d", "M-9.5,0L-3,3.5L-3,-3.5Z").call(d3version3.behavior.drag().on("drag", function(t, o) {
                t = (o ? i : u) + 90;
                var s = [-r * Math.cos(t * d), -r * Math.sin(t * d)],
                    l = [d3version3.event.x, d3version3.event.y],
                    c = ~~(Math.atan2(n(s, l), a(s, l)) / d);
                t = Math.max(-90, Math.min(90, t + c - 90)), c = i - u, o ? (i = t, c > 360 ? u += c - 360 : 0 > c && (u = i)) : (u = t, c > 360 ? i += 360 - c : 0 > c && (i = u)), e()
            }).on("dragend", generatePositive)), s.attr("transform", function(t) {
                return "rotate(" + (t + 90) + ")translate(-" + r + ")"
            }), pLayout.rotate(function() {
                return h(~~(Math.random() * c))
            }), d3version3.select("#angle-count").property("value", c), d3version3.select("#angle-from").property("value", u), d3version3.select("#angle-to").property("value", i)
        }
        ...
        var r = 40.5,
            o = 35,
            s = 20,
            l = d3version3.select("#angles").append("svg").attr("width", 2 * (r + o)).attr("height", r + 1.5 * s).append("g").attr("transform", "translate(" + [r + o, r + s] + ")");
        ...
        var u, i, c, d = Math.PI / 180,
            h = d3version3.scale.linear(),
            f = d3version3.svg.arc().innerRadius(0).outerRadius(r);
        d3version3.selectAll("#angle-count, #angle-from, #angle-to").on("change", t).on("mouseup", t), t(), parsePositiveText(d3version3.select("#positive_text").property("value"))
    }();